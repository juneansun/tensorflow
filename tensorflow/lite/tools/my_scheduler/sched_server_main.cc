...
class IProcessor {
    public:
        virtual int getNumClient() = 0;

        virtual void incClient(int model_idx) = 0;

        virtual void decClient(int model_idx) = 0;

        virtual int getWCET(int model_idx) = 0;
};
...
int gpuTable[NUM_TFLITE_MODEL][NUM_TFLITE_MODEL] {
    { 0,    53045,  1920,   920,    7474    },
    { 956,  0,      735,    457,    2413    },
    { 4180, 115701, 0,      194,    15581   },
    { 334,  75926,  518,    0,      10873   },
    { 974,  8923,   298,    212,    0       },
};

class ProcessorGPU : public IProcessor {
    public:
        ProcessorGPU(int pID) {
            processorID = pID;
        ...
        int getWCET(int model_idx) {
            ...
            for(int i = 0; i < NUM_TFLITE_MODEL; i++) {
#if 0 // DEBUG
                LOGD("(JBD) GPU mNumClient[%d]: %d, model_idx[%d], i[%d]", i, mNumClient[i], model_idx, i);

#endif
                if (mNumClient[i] > 0) {
#if 0 // DEBUG
                    LOGD("(JBD) gpuTable[%d][%i] = %d", model_idx, i, gpuTable[model_idx][i]);
                    LOGD("(JBD) adding for GPU:%d", (gpuTable[model_idx][i] * mNumClient[i]));
#endif
                    wcet += (gpuTable[model_idx][i] * mNumClient[i]);
                }
            }
...
void handleMessage() {
    ...
    while(true) {
        ...
        if (msg.model_idx == 3 && type == 2) {
            int TPU = 3;
            processorPerClients[msg.clfd] = TPU;
            write(msg.clfd, &TPU, sizeof(int));
            continue;
        }
